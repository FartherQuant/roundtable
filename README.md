...
多角色结构化讨论的 prompt 模板——让 LLM 扮演不同思维视角的角色，交叉质疑，形成共识。
...
- **深度圆桌**：5次LLM调用，2.5分钟出结果 → [templates/deep-roundtable.md](templates/deep-roundtable.md)
...
roundtable/
  ...
  output-schemas/         # 产出格式定义
    consensus-matrix.yaml
    actionable-signals.yaml
    blindspots-list.yaml
```
